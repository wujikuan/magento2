...
namespace Zou\BannerSlider\Setup;
...
use Zou\BannerSlider\Model\Config\Source\Template;
use Psr\Log\LoggerInterface;
use Zend_Db_Exception;

/**
 * Class InstallSchema
 * @package Zou\BannerSlider\Setup
 */
class InstallSchema implements InstallSchemaInterface
{
    ...
    public function install(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        if (!$installer->tableExists('zou_bannerslider_banner')) {
            $table = $installer->getConnection()
                ->newTable($installer->getTable('zou_bannerslider_banner'))
                ->addColumn(
                    'banner_id',
                    Table::TYPE_INTEGER,
                    null,
            ...
            $installer->getConnection()->addIndex(
                $installer->getTable('zou_bannerslider_banner'),
                $setup->getIdxName(
                    $installer->getTable('zou_bannerslider_banner'),
        ...
        if (!$installer->tableExists('zou_bannerslider_slider')) {
            $table = $installer->getConnection()
                ->newTable($installer->getTable('zou_bannerslider_slider'))
                ->addColumn(
                    'slider_id',
                    Table::TYPE_INTEGER,
                    null,
        ...
        if (!$installer->tableExists('zou_bannerslider_banner_slider')) {
            $table = $installer->getConnection()
                ->newTable($installer->getTable('zou_bannerslider_banner_slider'))
                ->addColumn(
                    'slider_id',
                    Table::TYPE_INTEGER,
                    null,
                ...
                ->addIndex($installer->getIdxName('zou_bannerslider_banner_slider', ['slider_id']), ['slider_id'])
                ->addIndex($installer->getIdxName('zou_bannerslider_banner_slider', ['banner_id']), ['banner_id'])
                ->addForeignKey(
                    $installer->getFkName(
                        'zou_bannerslider_banner_slider',
                        'slider_id',
                        'zou_bannerslider_slider',
                        'slider_id'
                    ),
                    'slider_id',
                    $installer->getTable('zou_bannerslider_slider'),
                    'slider_id',
    ...
    private function copyDemoImage()
    {
        try {
            ...
            $url            = 'zou/bannerslider/banner/demo/';
